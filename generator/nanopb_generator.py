...
class Enum(ProtoElement):
    ...
    def __str__(self):
        ...
        for index, (name, value) in enumerate(self.values):
            ...
            enum_values.append(
                "    %s = %d%s %s" % (name, value, comma, trailing_comment)
            )
...
class Method:
    def __init__(self, service_name, desc, method_options):
        """desc is MethodDescriptorProto"""
        self.service_name = service_name
        self.name = desc.name
        self.request_type = names_from_type_name(desc.input_type)
        self.response_type = names_from_type_name(desc.output_type)

    def __str__(self):
        return "    %s %s;" % (self.name, inflection.underscore(self.name))

    def types(self):
        return "typedef bool (*%s)(%s* %s, %s* %s);\n" % (
            self.name,
            self.request_type,
            inflection.underscore(str(self.request_type)),
            self.response_type,
            inflection.underscore(str(self.response_type)),
        )

    def get_dependencies(self):
        """Get list of type names used by this field."""
        if self.allocation == "STATIC":
            return [str(self.response_type), str(self.request_type)]
        else:
            return []
    ...
    def tags(self):
        ...
        identifier = "%s_%s_tag" % (self.service_name, self.name)
        return "#define %-40s %d\n" % (identifier, self.tag)


class Service(ProtoElement):
    def __init__(self, names, desc, service_options, index, comments):
        super(Service, self).__init__(self.SERVICE, index, comments)
        self.name = names
        self.methods = []
        self.desc = desc
        self.packed = service_options.packed_struct

        if desc is not None:
            self.load_methods(desc, service_options)

    def load_methods(self, desc, service_options):
        for m in desc.method:
            method_options = get_nanopb_suboptions(
                m, service_options, self.name + m.name
            )
            if method_options.type == nanopb_pb2.FT_IGNORE:
                continue

            method = Method(self.name, m, method_options)
    ...
    def __str__(self):
        service_path = self.element_path
        leading_comment, trailing_comment = self.get_comments(
            service_path, leading_indent=False
        )
        ...
        for index, method in enumerate(self.methods):
            ...
            if leading_comment:
                service_methods.append(leading_comment)

            service_methods.append("%s %s" % (str(method), trailing_comment))
    ...
    def types(self):
        _ = "".join([m.types() for m in self.methods])
        _ += "\n"
        return _

    def get_initializer(self, null_init):
        return "{" + ", ".join(["NULL"] * len(self.methods)) + "}"

    def count_required_methods(self):
        return self.count_all_fields()
    ...
    def method_for_tag(self, tag):
        """Given a tag number, return the Method instance."""
...
class Field(ProtoElement):
    ...
    def __str__(self):
        ...
        if self.allocation == "POINTER":
            ...
            elif self.rules in ["REPEATED", "FIXARRAY"] and self.pbtype in [
                "STRING",
                "BYTES",
            ]:
...
class Message(ProtoElement):
    ...
    def load_fields(self, desc, message_options):
        ...
        for index, f in enumerate(desc.field):
            ...
            field = Field(
                self.name, f, field_options, self.element_path, index, self.comments
            )
...
class ProtoFile:
    ...
    def parse(self):
        ...
        for index, enum in enumerate(self.fdesc.enum_type):
            ...
            self.enums.append(
                Enum(name, enum, enum_options, index, self.comment_locations)
            )

        for index, (names, message) in enumerate(
            iterate_messages(self.fdesc, self.manglenames.flatten)
        ):
            ...
            self.messages.append(
                Message(name, message, message_options, index, self.comment_locations)
            )
        ...
        for index, service in enumerate(self.fdesc.service):
            ...
            service_options = get_nanopb_suboptions(service, self.file_options, name)
            self.services.append(
                Service(name, service, service_options, index, self.comment_locations)
            )
    ...
    def generate_header(self, includes, headername, options):
        ...
        if self.services:
            yield "/* Service definitions */\n"
            for service in self.services:
                yield service.types()
                yield str(service) + "\n"
                identifier = "%s_init_default" % service.name
                yield "#define %-40s %s\n" % (
                    identifier,
                    service.get_initializer(False),
                )
            yield "\n"
...
def get_nanopb_suboptions(subdesc, options, name):
    ...
    elif isinstance(subdesc.options, descriptor.ServiceOptions):
        ext_type = nanopb_pb2.nanopb_serviceopt
    elif isinstance(subdesc.options, descriptor.MethodOptions):
        ext_type = nanopb_pb2.nanopb_methodopt
    else:
...
optparser.add_option(
    "--version", dest="version", action="store_true", help="Show version info and exit"
)
optparser.add_option(
    "-x",
    dest="exclude",
...
def main_plugin():
    ...
    optparser.usage = (
        "Usage: protoc --nanopb_out=[options][,more_options]:outdir file.proto"
    )
